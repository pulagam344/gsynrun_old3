...
wget -O swarm.pem https://raw.githubusercontent.com/pulagam344/swarm_peers/main/25/swarm.pem
mkdir -p configs
...
# Part 2
git clone https://github.com/gensyn-ai/rl-swarm.git /root/my_rl_swarm_26
cd /root/my_rl_swarm_26
rm -f run_rl_swarm.sh && wget -O run_rl_swarm.sh https://raw.githubusercontent.com/pulagam344/gsyn_runsh/main/run_rl_swarm.sh && chmod +x run_rl_swarm.sh
...
wget -O swarm.pem https://raw.githubusercontent.com/pulagam344/swarm_peers/main/26/swarm.pem
mkdir -p configs
wget -O configs/rg-swarm.yaml https://raw.githubusercontent.com/pulagam344/gsyn_connfig/main/rg-swarm.yaml
...
monitor_swarms() {
  while true; do
    ...
    if [ ! -f "/root/running_3001.txt" ]; then
      ...
      run_swarm "/root/my_rl_swarm_25" 0 "swarm_25" &
    fi

    # Check swarm_26
    if [ ! -f "/root/running_3002.txt" ]; then
      echo "[$(date +%H:%M:%S)]-[monitor] Swarm_26 stopped, restarting..."
      run_swarm "/root/my_rl_swarm_26" 1 "swarm_26" &
    fi
